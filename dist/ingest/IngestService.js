...
class IngestService {
     ...
     */
    static VALID_CHANNELS = [PROGRESS_CHANNEL];
    /**
     ...
     */
    static MAX_CHANNEL_LENGTH = DEFAULT_MAX_CHANNEL_LENGTH;
    messageRouter;
    logger;
    metricsCollector;
    clientChannels;
    ...
    async handleAction(clientId, action, data) {
        try {
            ...
            this.logger.error(`Error handling ingest action ${action} for client ${clientId}:`, error);
    ...
    async handleSubscribe(clientId, { channel }) {
        if (!this._isValidChannel(channel)) {
            this.sendError(clientId, 'channel is required (ingest:progress | ingest:source:{sourceId})');
            return;
        }
    ...
    async emitEvent(channel, event) {
        ...
        if (this.messageRouter) {
            try {
                ...
                this.logger.error(`[ingest] Failed to broadcast event to ${channel}:`, err && err.message ? err.message : err);
            }
        }
        else {
    ...
    _isValidChannel(channel) {
        if (!channel || typeof channel !== 'string')
            return false;
        if (channel.length === 0 || channel.length > this.maxChannelLength)
            return false;
        if (channel === PROGRESS_CHANNEL)
            return true;
        if (channel.startsWith(SOURCE_CHANNEL_PREFIX) &&
    ...
    async handleDisconnect(clientId) {
        ...
        for (const channel of channels) {
            try {
                ...
                this.logger.error(`Error unsubscribing client ${clientId} from ingest channel ${channel}:`, error);
            }
        }
...
exports.default = IngestService;
//# sourceMappingURL=IngestService.js.map