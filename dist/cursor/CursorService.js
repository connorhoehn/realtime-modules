...
class CursorService {
    ...
    constructor(opts) {
        ...
        this.throttleInterval = config.throttleInterval ?? types_1.DEFAULT_THROTTLE_INTERVAL_MS;
        this.cursorTTL = config.cursorTTL ?? types_1.DEFAULT_CURSOR_TTL_MS;
        this.cleanupInterval = config.cleanupInterval ?? types_1.DEFAULT_CLEANUP_INTERVAL_MS;
    ...
    async cleanupRoom(roomId) {
        ...
        this.logger.info(`cursor-service flushed cursor state for roomId ${roomId} (${cleared} cursor(s) cleared)`);
    }
    ...
    validatePositionForMode(position, mode) {
        ...
        switch (mode) {
            ...
            case 'table':
                return (typeof position.row === 'number' &&
    ...
    async handleGetModes(clientId, _data) {
