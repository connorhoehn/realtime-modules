...
exports.FileUploadService = exports.InMemoryFileUploadMetadataStore = void 0;
const crypto_1 = require("crypto");
const FileBlobStore_1 = require("./FileBlobStore");
const FileBlobStore_2 = require("./FileBlobStore");
...
class InMemoryFileUploadMetadataStore {
    rows = new Map();
    async create(row) {
        this.rows.set(row.uploadId, { ...row, createdAt: row.createdAt ?? new Date().toISOString() });
    }
    async get(uploadId) {
    ...
    async updateStatus(uploadId, status, patch = {}) {
        ...
        if (row)
            this.rows.set(uploadId, { ...row, ...patch, status, updatedAt: new Date().toISOString() });
    }
}
exports.InMemoryFileUploadMetadataStore = InMemoryFileUploadMetadataStore;
...
class FileUploadService {
    ...
    constructor(opts) {
        ...
        this.blobStore = opts.blobStore ?? new FileBlobStore_1.FileBlobStore();
     ...
     * unresolvable.
     */
    async resolveByCorrelation(clientId, channel, correlationId) {
        const mintedId = this.correlationToUploadId.get(this.correlationKey(clientId, channel, correlationId));
        if (!mintedId)
            return null;
    ...
    emitFailed(clientId, channel, id, error) {
     ...
     */
    sendError(clientId, message, code = AUTHZ_CHANNEL_DENIED) {
        void this.messageRouter.sendToClient(clientId, {
            type: 'fileupload:failed',
            service: 'fileupload',
            code,
    ...
    async handleAction(clientId, action, data) {
        ...
        if (!id || !(0, FileBlobStore_2.sanitizeUploadId)(id)) {
    ...
    async handleRequestUpload(clientId, channel, id, data) {
        ...
        this.correlationToUploadId.set(this.correlationKey(clientId, channel, correlationId), uploadId);
        ...
        void this.messageRouter.sendToChannel(channel, {
            ...
            ...(height ? { height } : {}),
            timestamp: new Date().toISOString(),
        }, null, { publisherClientId: clientId });
    }
    /**
     ...
     */
    publishProgress(uploadId, transferred) {
        const active = this.activeTransfers.get(uploadId);
        if (!active)
            return;
        void this.messageRouter.sendToChannel(active.channel, {
            ...
            size: active.size,
            timestamp: new Date().toISOString(),
        }, null, { publisherClientId: active.clientId });
    }
    /**
     ...
     */
    publishSettled(uploadId, phase, error) {
        const active = this.activeTransfers.get(uploadId);
        if (!active)
            return;
    ...
    describeTransfer(uploadId) {
        return this.activeTransfers.get(uploadId) ?? null;
    }
    async handleComplete(clientId, channel, id) {
    ...
    async handleCancel(clientId, channel, id) {
        const resolved = await this.resolveByCorrelation(clientId, channel, id);
        if (resolved) {
            const { uploadId, row } = resolved;
            ...
            if (row.status !== 'completed') {
                await this.metadataRepo.updateStatus(uploadId, 'cancelled').catch(() => { });
    ...
    onClientDisconnect(clientId) {
        ...
        for (const key of Array.from(this.correlationToUploadId.keys())) {
            if (key.startsWith(prefix))
                this.correlationToUploadId.delete(key);
        }
