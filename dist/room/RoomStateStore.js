...
exports.RedisRoomStateStore = exports.InMemoryRoomStateStore = void 0;
class InMemoryRoomStateStore {
    rooms = new Map();
    async addMember(slug, clientId, userId, displayName, participantId) {
    ...
    async removeMember(slug, clientId) {
    ...
    async countMembers(slug) {
        return this.rooms.get(slug)?.size ?? 0;
    }
    async listAllRoomCounts(limit = 200) {
        const out = [];
        let n = 0;
    ...
    async touch(_slug, _clientId) {
        // In-memory store has no TTL — no-op.
    }
}
exports.InMemoryRoomStateStore = InMemoryRoomStateStore;
...
class RedisRoomStateStore {
    ...
    membersKey(slug) { return `${ROOM_MEMBERS_PREFIX}${slug}`; }
    clientKey(clientId) { return `${ROOM_CLIENT_PREFIX}${clientId}`; }
    ...
    async listAllRoomCounts(limit = 200) {
        ...
        do {
            ...
            const result = await r.scan(cursor, { MATCH: `${ROOM_MEMBERS_PREFIX}*`, COUNT: SCAN_BATCH });
            let keys = [];
...
exports.RedisRoomStateStore = RedisRoomStateStore;
//# sourceMappingURL=RoomStateStore.js.map