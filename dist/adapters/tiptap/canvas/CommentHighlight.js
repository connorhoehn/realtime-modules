...
exports.CommentHighlight = exports.commentHighlightPluginKey = exports.CANVAS_COMMENT_CLASS = void 0;
exports.pmRangesFromPlain = pmRangesFromPlain;
exports.plainRangeFromPm = plainRangeFromPm;
...
exports.CANVAS_COMMENT_CLASS = 'canvas-comment';
/** Matches `CANVAS_BODY_KEY` in `useCanvasDocument`; not imported — that module is a React hook. */
const DEFAULT_FRAGMENT_KEY = 'body';
exports.commentHighlightPluginKey = new state_1.PluginKey('canvasCommentHighlight');
/**
 ...
 */
function toPmRanges(segments, from, to) {
    const ranges = [];
 ...
 */
function pmRangesFromPlain(doc, from, to) {
 ...
 * this number line.
 */
function plainRangeFromPm(doc, from, to) {
    if (to <= from)
        return null;
    let start = null;
    let end = null;
    for (const segment of textSegments(doc)) {
        ...
        if (start === null)
            start = segment.plain + Math.max(from - segment.pm, 0);
 ...
 */
function classesFor(prefix, state) {
...
function buildDecorations(doc, ydoc, options, comments) {
    if (!ydoc || comments.length === 0)
    ...
    for (const comment of comments) {
        ...
        if (!(0, anchors_1.isCanvasAnchor)(anchor) || anchor.key !== options.fragmentKey)
            continue;
        if (options.hideResolved && comment.state === 'resolved')
            continue;
        let range = null;
        try {
            range = (0, anchors_1.resolveAnchor)(ydoc, anchor);
        }
        catch {
        ...
        for (const pmRange of toPmRanges(segments, range.from, range.to)) {
            decorations.push(view_1.Decoration.inline(pmRange.from, pmRange.to, attributes, {
...
exports.CommentHighlight = core_1.Extension.create({
    ...
    addOptions() {
        return {
            ...
            comments: [],
            hideResolved: false,
            classPrefix: exports.CANVAS_COMMENT_CLASS,
            onCommentClick: null,
        };
    },
    addCommands() {
        return {
            setCommentHighlights: (comments) => ({ tr, dispatch }) => {
                if (dispatch) {
                    dispatch(tr.setMeta(exports.commentHighlightPluginKey, { comments }));
                }
                return true;
            },
            refreshCommentHighlights: () => ({ tr, dispatch }) => {
                if (dispatch)
                    dispatch(tr.setMeta(exports.commentHighlightPluginKey, {}));
                return true;
            },
            setCommentHighlightDocument: (ydoc) => ({ tr, dispatch }) => {
                if (dispatch) {
                    dispatch(tr.setMeta(exports.commentHighlightPluginKey, { ydoc }));
                }
                return true;
            },
        };
    ...
    addProseMirrorPlugins() {
        ...
        return [
            new state_1.Plugin({
                key: exports.commentHighlightPluginKey,
                state: {
                    init(_config, state) {
                        return {
                            ydoc: options.ydoc,
                    ...
                    apply(tr, previous) {
                        const meta = tr.getMeta(exports.commentHighlightPluginKey);
                        if (meta) {
                ...
                props: {
                    decorations(state) {
                        return exports.commentHighlightPluginKey.getState(state)?.decorations ?? view_1.DecorationSet.empty;
                    },
                    handleClick(view, pos, event) {
                        const onCommentClick = options.onCommentClick;
                        if (!onCommentClick)
                            return false;
                        const decorations = exports.commentHighlightPluginKey.getState(view.state)?.decorations;
                        if (!decorations)
                            return false;
...
//# sourceMappingURL=CommentHighlight.js.map