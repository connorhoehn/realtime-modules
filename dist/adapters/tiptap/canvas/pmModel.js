...
function blockToPm(block, unsupported, nodes) {
    switch (block.type) {
        case 'heading': {
...
function listToPm(ordered, start, items, unsupported, nodes) {
...
function listItemBlocksToPm(blocks, unsupported, nodes) {
...
function blocksToPm(blocks, unsupported, nodes) {
    const out = [];
...
function docModelToPm(model, schema) {
    const unsupported = [];
...
function tableToModel(node) {
    const rows = node.content ?? [];
    const cells = (row) => (row.content ?? []).map(cell => {
        const content = [];
        for (const block of pmBlocksToModel(cell.content)) {
            ...
            if (block.type === 'paragraph' || block.type === 'heading')
                ...
                content.push({ type: 'text', value: (0, document_1.serializeDocument)({ frontMatter: {}, content: [block] }).trimEnd() });
        }
        return content;
    });
