...
class CallService {
     ...
     * announce.
     */
    _announceCallEnded(callId, state, missedReason = 'cancelled', endedAtHint) {
        ...
        const localKnown = new Set([...(state.everParticipated ?? []), ...state.participantClientIds]);
        ...
        })().catch((e) => this.logger.warn(`[CallService] call-ended/missed hook failed for ${callId}: ${e?.message ?? e}`));
    }
    /**
    ...
    forgetCall(callId, missedReason = 'cancelled', endedAt) {
