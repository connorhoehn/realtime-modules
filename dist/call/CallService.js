...
class CallService {
    ...
    async handleCallEvent(clientId, action, data) {
        ...
        if (targetUserIds.length > 0) {
            ...
            if (failures.length > 0) {
                this.logger.warn(`Client ${clientId} call event '${action}': ${delivered}/${planned} delivered ` +
                    `(callId=${callId ?? '-'} lobby=${lobbyName ?? '-'})`, { failures });
            }
            else {
                this.logger.info(`Client ${clientId} sent call event '${action}' to users [${targetUserIds.join(', ')}] ` +
     ...
     */
    findClientsForUsers(userIds, excludeClientId) {
    ...
    async handleDisconnect(_clientId) {
