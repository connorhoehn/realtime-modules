...
exports.jwtSecondsRemaining = jwtSecondsRemaining;
/**
 ...
 */
function decodeArn(token) {
    const claims = decodeJwt(token);
    if (!claims)
        return null;
    if (typeof claims.arn === 'string' && claims.arn.length > 0)
        return claims.arn;
    if (typeof claims.channelArn === 'string' && claims.channelArn.length > 0)
        return claims.channelArn;
    return null;
}
/**
 ...
 */
function jwtSecondsRemaining(token, now = new Date()) {
    const claims = decodeJwt(token);
    if (!claims)
        return null;
