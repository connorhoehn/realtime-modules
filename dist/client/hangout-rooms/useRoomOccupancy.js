...
exports.useRoomOccupancy = useRoomOccupancy;
 ...
 * ```
 */
function useRoomOccupancy(slug, opts = {}) {
    const [occupancy, setOccupancy] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        ...
        if (!slug)
            return;
        if (!opts.subscribeRoomsIndex)
            return;
        const unsubscribe = opts.subscribeRoomsIndex((occ) => {
            if (!occ || occ.slug !== slug)
                return;
            setOccupancy(occ);
        });
        return unsubscribe;
...
//# sourceMappingURL=useRoomOccupancy.js.map