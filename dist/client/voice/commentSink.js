...
exports.evaluateAttach = evaluateAttach;
exports.attachTranscriptAsComment = attachTranscriptAsComment;
/**
 ...
 */
function evaluateAttach(event, confirmed = false) {
    const { context, outcome, text } = event;
    if (outcome === 'lost') {
        return {
            ok: false,
            reason: 'transcript-lost',
            detail: 'audio was captured but no transcript came back — the transcription queue may have dropped it',
        };
    }
    if (!text.trim()) {
 ...
 */
async function attachTranscriptAsComment(event, opts) {
...
//# sourceMappingURL=commentSink.js.map