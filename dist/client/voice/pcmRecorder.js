...
class PcmRecorder {
    ...
    constructor(opts) {
        this.opts = {
            ...
            silenceRms: opts.silenceRms ?? pcm_1.DEFAULT_SILENCE_RMS,
    ...
    async start() {
        ...
        try {
            ...
            const ctx = new AudioContext({ sampleRate: pcm_1.TARGET_SAMPLE_RATE });
            this.ctx = ctx;
            ...
            const chunkFrames = Math.max(128, Math.round((this.opts.chunkMs / 1000) * ctx.sampleRate));
            ...
            node.port.onmessage = (ev) => {
                try {
                    this.emit(ev.data, ctx.sampleRate);
                }
                catch (err) {
    ...
    emit(samples, ctxRate) {
        const at16k = ctxRate === pcm_1.TARGET_SAMPLE_RATE
            ? samples
            : (0, pcm_1.resampleLinear)(samples, ctxRate, pcm_1.TARGET_SAMPLE_RATE);
        const s16 = (0, pcm_1.floatToS16)(at16k);
        const rms = (0, pcm_1.rmsS16)(s16);
        this.opts.onChunk({
            bytes: (0, pcm_1.s16ToBytes)(s16),
            durationMs: (s16.length / pcm_1.TARGET_SAMPLE_RATE) * 1000,
            rms,
