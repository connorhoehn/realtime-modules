...
exports.isWorkReference = exports.isWorkGraphDeltaBatch = exports.isWorkGraphSnapshot = exports.isWorkHistoryGrant = exports.isWorkSharingGrant = exports.isAuthenticatedWorkEvent = void 0;
exports.validateAuthenticatedWorkEvent = validateAuthenticatedWorkEvent;
exports.validateWorkSharingGrant = validateWorkSharingGrant;
exports.validateWorkHistoryGrant = validateWorkHistoryGrant;
exports.validateWorkGraphSnapshot = validateWorkGraphSnapshot;
exports.validateWorkGraphDeltaBatch = validateWorkGraphDeltaBatch;
exports.validateWorkReference = validateWorkReference;
/**
...
const disclosures = new Set(['existence', 'summary', 'details']);
...
function stringWithin(value, max = contracts_1.WORK_GRAPH_LIMITS.idLength) {
...
function isTimezone(value) {
    if (typeof value !== 'string' || value.length === 0 || value.length > contracts_1.WORK_GRAPH_LIMITS.timezoneLength)
        return false;
...
function hasUniqueIds(values, maximum) {
...
function withinSerializedLimit(value, maximum) {
    ...
    switch (source) {
        ...
        case 'document':
            return hasExactKeys(value, ['kind', 'lifecycle', 'documentId', 'revisionId'], ['attributedActorId', 'producedByRunId', 'safeLabel'])
                && ['revision-saved', 'deleted'].includes(value.lifecycle)
        ...
        case 'conversation':
            return hasExactKeys(value, ['kind', 'lifecycle', 'conversationId', 'conversationKind'], ['explicitRelatedResource', 'safeLabel'])
                ...
                && isId(value.conversationId) && ['dm', 'channel'].includes(value.conversationKind)
...
function isViewerNode(value) {
    return hasExactKeys(value, ['id', 'kind', 'title', 'status', 'disclosure', 'updatedAt', 'capabilities'], ['description', 'startedAt', 'endedAt', 'locked'])
        && isId(value.id) && nodeKinds.has(value.kind)
        && stringWithin(value.title, contracts_1.WORK_GRAPH_LIMITS.labelLength)
        && (value.description === undefined || stringWithin(value.description, contracts_1.WORK_GRAPH_LIMITS.descriptionLength))
        && statuses.has(value.status) && disclosures.has(value.disclosure)
        && Array.isArray(value.capabilities) && value.capabilities.every((capability) => capabilities.has(capability))
...
function isViewerEdge(value) {
    return hasExactKeys(value, ['id', 'fromId', 'toId', 'relation', 'status', 'disclosure', 'updatedAt', 'capabilities'], ['label', 'startedAt', 'endedAt'])
        ...
        && relations.has(value.relation) && statuses.has(value.status)
        && disclosures.has(value.disclosure)
        && (value.label === undefined || stringWithin(value.label, contracts_1.WORK_GRAPH_LIMITS.labelLength))
        && Array.isArray(value.capabilities) && value.capabilities.every((capability) => capabilities.has(capability))
        && new Set(value.capabilities).size === value.capabilities.length
        && (value.startedAt === undefined || isTimestamp(value.startedAt))
        && isTimestamp(value.updatedAt)
...
function isSourceStatus(value) {
    return hasExactKeys(value, ['source', 'health', 'checkedAt'], ['message'])
        && sourceKinds.has(value.source) && sourceHealth.has(value.health)
        && isTimestamp(value.checkedAt)
        && (value.message === undefined || stringWithin(value.message, contracts_1.WORK_GRAPH_LIMITS.descriptionLength));
}
function isReferenceTarget(value) {
    return hasExactKeys(value, ['kind', 'id']) && (value.kind === 'node' || value.kind === 'edge') && isId(value.id);
}
function result(valid, value, name) {
    return valid ? { ok: true, value: value } : { ok: false, errors: [`Invalid ${name}.`] };
}
function validateAuthenticatedWorkEvent(value) {
    const valid = hasExactKeys(value, ['schemaVersion', 'eventId', 'idempotencyKey', 'source', 'occurredAt', 'receivedAt', 'actor', 'producer', 'resource', 'payload'], ['sourceSequence'])
        ...
        && sourceKinds.has(value.source) && isTimestamp(value.occurredAt) && isTimestamp(value.receivedAt)
        ...
        && isSourceRef(value.resource, value.source)
        && isEventPayload(value.payload, value.source)
        && withinSerializedLimit(value, contracts_1.WORK_GRAPH_LIMITS.sourceEventBytes);
    return result(valid, value, 'authenticated work event');
}
function isGrantBase(value, history) {
    const optional = history ? ['dayFrom', 'dayThrough'] : ['historyGrantId'];
    if (!hasExactKeys(value, ['schemaVersion', 'id', 'organizationId', 'ownerId', 'audience', 'selection', 'state', 'createdAt', 'updatedAt', 'expiresAt', 'revision'], optional))
        return false;
    if (!isSchemaVersion(value.schemaVersion) || !isId(value.id) || !isId(value.organizationId) || !isId(value.ownerId)
        || !grantStates.has(value.state) || !isTimestamp(value.createdAt) || !isTimestamp(value.updatedAt)
        || !isTimestamp(value.expiresAt) || !isPositiveInteger(value.revision))
        return false;
    ...
    const maximum = history ? contracts_1.WORK_GRAPH_LIMITS.maximumHistorySharingMs : contracts_1.WORK_GRAPH_LIMITS.maximumSharingMs;
    if (duration < contracts_1.WORK_GRAPH_LIMITS.minimumSharingMs || duration > maximum)
        return false;
    if (!isSharingAudience(value.audience) || !isSharingSelection(value.selection))
        return false;
    return history || value.historyGrantId === undefined || isId(value.historyGrantId);
}
function isSharingAudience(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
        return false;
    if (value.kind === 'organization')
        return hasExactKeys(value, ['kind']);
    if (value.kind === 'people')
        return hasExactKeys(value, ['kind', 'personIds']) && hasUniqueIds(value.personIds, contracts_1.WORK_GRAPH_LIMITS.pageNodes) && value.personIds.length > 0;
    return value.kind === 'conversation' && hasExactKeys(value, ['kind', 'conversationIds']) && hasUniqueIds(value.conversationIds, contracts_1.WORK_GRAPH_LIMITS.pageNodes) && value.conversationIds.length > 0;
}
function isSharingSelection(value) {
    return hasExactKeys(value, ['nodeIds', 'edgeIds', 'disclosure'])
        && hasUniqueIds(value.nodeIds, contracts_1.WORK_GRAPH_LIMITS.snapshotNodes)
        && hasUniqueIds(value.edgeIds, contracts_1.WORK_GRAPH_LIMITS.snapshotEdges)
        && value.nodeIds.length + value.edgeIds.length > 0
        && disclosures.has(value.disclosure);
}
function validateWorkSharingGrant(value) {
    return result(isGrantBase(value, false), value, 'work sharing grant');
}
function validateWorkHistoryGrant(value) {
    const valid = hasExactKeys(value, ['schemaVersion', 'id', 'organizationId', 'ownerId', 'audience', 'selection', 'state', 'createdAt', 'updatedAt', 'expiresAt', 'revision', 'dayFrom', 'dayThrough'])
        ...
        && Date.parse(`${value.dayThrough}T00:00:00.000Z`) - Date.parse(`${value.dayFrom}T00:00:00.000Z`) < contracts_1.WORK_GRAPH_LIMITS.maximumHistorySharingMs;
    return result(valid, value, 'work history grant');
}
function validateWorkGraphSnapshot(value) {
    let valid = hasExactKeys(value, ['schemaVersion', 'scope', 'revision', 'watermark', 'cursor', 'nodes', 'edges', 'sources', 'partial'], ['nextPageCursor'])
        ...
        && Array.isArray(value.nodes) && value.nodes.length <= contracts_1.WORK_GRAPH_LIMITS.snapshotNodes && value.nodes.every(isViewerNode)
        && Array.isArray(value.edges) && value.edges.length <= contracts_1.WORK_GRAPH_LIMITS.snapshotEdges && value.edges.every(isViewerEdge)
...
function isDeltaOperation(value) {
    if (!isRecord(value) || typeof value.kind !== 'string')
        return false;
    switch (value.kind) {
...
function validateWorkGraphDeltaBatch(value) {
    const valid = hasExactKeys(value, ['schemaVersion', 'subscriptionGeneration', 'previousWatermark', 'watermark', 'cursor', 'policyRevision', 'operations'])
        ...
        && Array.isArray(value.operations) && value.operations.length <= contracts_1.WORK_GRAPH_LIMITS.deltaOperations
        && value.operations.every(isDeltaOperation) && withinSerializedLimit(value, contracts_1.WORK_GRAPH_LIMITS.deltaBytes);
    return result(valid, value, 'work graph delta batch');
}
function validateWorkReference(value) {
    const valid = hasExactKeys(value, ['version', 'personId', 'day', 'timezone', 'target'], ['sessionId', 'eventId', 'observedAt'])
        ...
        && withinSerializedLimit(value, contracts_1.WORK_GRAPH_LIMITS.referenceBytes);
    return result(valid, value, 'work reference');
}
const isAuthenticatedWorkEvent = (value) => validateAuthenticatedWorkEvent(value).ok;
exports.isAuthenticatedWorkEvent = isAuthenticatedWorkEvent;
const isWorkSharingGrant = (value) => validateWorkSharingGrant(value).ok;
exports.isWorkSharingGrant = isWorkSharingGrant;
const isWorkHistoryGrant = (value) => validateWorkHistoryGrant(value).ok;
exports.isWorkHistoryGrant = isWorkHistoryGrant;
const isWorkGraphSnapshot = (value) => validateWorkGraphSnapshot(value).ok;
exports.isWorkGraphSnapshot = isWorkGraphSnapshot;
const isWorkGraphDeltaBatch = (value) => validateWorkGraphDeltaBatch(value).ok;
exports.isWorkGraphDeltaBatch = isWorkGraphDeltaBatch;
const isWorkReference = (value) => validateWorkReference(value).ok;
exports.isWorkReference = isWorkReference;
