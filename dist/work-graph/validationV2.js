...
function validateWorkGraphSnapshotV2(value) {
    ...
    if (snapshot.temporal.mode === 'as-of' && snapshot.temporal.coverage.complete && (snapshot.temporal.coverage.from > snapshot.query.windowStart || snapshot.temporal.coverage.through < snapshot.query.windowEnd))
        valid = false;
    if (snapshot.temporal.mode === 'as-of' && [...snapshot.nodes, ...snapshot.edges].some((entity) => entity.updatedAt > snapshot.query.windowEnd || (entity.startedAt && entity.startedAt > snapshot.query.windowEnd) || (entity.endedAt && entity.endedAt > snapshot.query.windowEnd)))
        valid = false;
    ...
    for (const operation of snapshot.operations) {
        ...
        if (!process || !['run', 'agent', 'terminal'].includes(process.kind) || !edge || ![edge.fromId, edge.toId].includes(process.id) || (snapshot.temporal.mode === 'as-of' && operation.observedAt > snapshot.query.windowEnd))
            valid = false;
    }
    const bucketTimes = new Set();
    for (const bucket of eventBuckets) {
        if (!exact(bucket, ['at', 'count']) || !time(bucket.at) || !integer(bucket.count) || bucket.count === 0 || bucketTimes.has(bucket.at) || bucket.at > snapshot.temporal.observedAt || (snapshot.temporal.mode === 'as-of' && bucket.at > snapshot.query.windowEnd)) {
            valid = false;
            break;
        }
