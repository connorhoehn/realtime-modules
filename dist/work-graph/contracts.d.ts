...
export declare const WORK_GRAPH_LIMITS: {
    ...
    readonly sourceEventBytes: number;
    readonly referenceBytes: number;
    readonly pageNodes: 100;
    readonly pageEdges: 200;
    readonly snapshotNodes: 250;
    readonly snapshotEdges: 500;
    readonly deltaOperations: 200;
    readonly deltaBytes: number;
    readonly replayBatches: 1000;
    readonly replayAgeMs: number;
    readonly heartbeatFreshMs: number;
    readonly heartbeatStaleMs: number;
    readonly eventRetentionDays: 30;
    readonly minimumSharingMs: number;
    readonly maximumSharingMs: number;
    readonly maximumHistorySharingMs: number;
    readonly maximumCollaborationRequestMs: number;
};
...
export type WorkSourceKind = 'cloud-compute' | 'local-compute' | 'document' | 'pipeline' | 'conversation' | 'meeting';
export type WorkNodeKind = 'project' | 'terminal' | 'agent' | 'run' | 'document' | 'change' | 'conversation' | 'meeting' | 'transcript' | 'task';
export type WorkRelation = 'works-on' | 'runs-in' | 'operates-on' | 'produced' | 'edited' | 'attended' | 'discussed' | 'derived-from';
/** Work state and sharing freshness are intentionally independent. */
export type WorkStatus = 'idle' | 'running' | 'waiting' | 'completed' | 'error' | 'stopped' | 'stale';
export type WorkSourceHealth = 'available' | 'delayed' | 'disconnected' | 'authorization-unavailable';
export type DisclosureLevel = 'existence' | 'summary' | 'details';
export type PolicyDecision = 'allow' | 'deny' | 'unavailable';
export type CollaborationCapability = 'message' | 'view-document' | 'edit-document' | 'join-conversation' | 'join-meeting' | 'observe-terminal' | 'control-terminal' | 'view-transcript' | 'view-run' | 'dispatch-run';
export type CollaborationRequestState = 'requested' | 'accepted' | 'declined' | 'expired' | 'revoked';
...
export type WorkSourcePayload = CloudComputePayload | LocalComputePayload | DocumentPayload | PipelinePayload | ConversationPayload | MeetingPayload;
export type AuthenticatedWorkEvent = WorkEventBase & {
    payload: WorkSourcePayload;
};
...
export type WorkGraphStreamMessage = {
    ...
    kind: 'invalidate';
    subscriptionGeneration: string;
    reason: 'policy-changed' | 'sharing-paused' | 'sharing-stopped' | 'sharing-expired' | 'cursor-expired' | 'source-authorization-unavailable';
} | {
    kind: 'reset-required';
    subscriptionGeneration: string;
    reason: 'gap' | 'replay-unavailable' | 'scope-changed';
};
...
export interface WorkSharingRepository {
    ...
    listEffectiveGrants(organizationId: string, ownerId: string, viewer: WorkSharingViewerContext, now: IsoTimestamp): Promise<Array<WorkSharingGrant | WorkHistoryGrant>>;
    putGrant(grant: WorkSharingGrant | WorkHistoryGrant, expectedRevision: number | null): Promise<WorkSharingGrant | WorkHistoryGrant>;
}
...
export interface WorkSourceAdapter<TRecord = unknown> {
    readonly source: WorkSourceKind;
    map(record: TRecord, producer: AuthenticatedWorkProducer, receivedAt: IsoTimestamp): AuthenticatedWorkEvent | {
        unsupported: true;
        reason: string;
    };
}
