...
class CRDTService {
    ...
    // ===================================================================
    async handleCopyDocument(clientId, data) {
        const requestId = typeof data?.requestId === 'string' ? data.requestId : undefined;
        const sourceId = typeof data?.documentId === 'string' ? data.documentId : '';
        const fail = (code, error) => this.sendToClient(clientId, { type: 'crdt', action: 'documentCopyFailed', requestId, sourceId, error, code });
        const sourceChannel = `doc:${sourceId}`;
        if (!sourceId || !this._validateChannel(sourceChannel))
            return fail('invalid_request', 'documentId is required');
        if (data.title !== undefined && (typeof data.title !== 'string' || !data.title.trim()))
            return fail('invalid_request', 'title must be a non-empty string');
        ...
        try {
            ...
            if (!await this.authorize(clientId, '', 'create', false))
                return fail('forbidden', 'Not allowed to create documents');
            const source = await this.metadataService.handleGetDocument(sourceId);
            if (!source)
                return fail('not_found', 'Source document not found');
            ...
            state.ydoc.transact(() => {
                if (meta.has('id'))
                    meta.set('id', doc.id);
                if (meta.has('documentId'))
                    meta.set('documentId', doc.id);
                if (meta.has('title'))
                    meta.set('title', title);
                // A copy was not imported from the source's seed revision.
                if (meta.has('importSourceRevision'))
                    meta.delete('importSourceRevision');
            });
            state.operationsSinceSnapshot++;
            await this.snapshotManager.writeSnapshot(channel);
            ...
            if (createdId) {
                ...
                try {
                    await this.metadataService.handleDeleteDocument(createdId);
                }
                catch (cleanupErr) {
                    this.logger.error(`copyDocument cleanup of ${createdId} failed:`, cleanupErr);
                }
            }
    ...
    // ===================================================================
    async ensureHydratedState(channel) {
        ...
        const load = (async () => {
            try {
                ...
                if (this.channelStates.get(channel) === state)
                    this.channelStates.delete(channel);
                state.ydoc.destroy();
                throw error;
            }
        ...
        try {
            ...
            if (this.hydration.get(channel) === load)
                this.hydration.delete(channel);
        }
    }
                ...
                if (state.subscriberCount <= 0) {
                    ...
                    if (state.operationsSinceSnapshot > 0) {
                        try {
                            ...
                            this.logger.error('Snapshot on unsubscribe failed; retaining dirty state', error);
                        }
                    }
