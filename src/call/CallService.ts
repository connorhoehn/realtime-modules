...
import {
    ...
    type CallWithSpan,
    type CallSweeperIsLeader,
    type DocumentCallInvite,
    type DocumentCallMeta,
    type DocumentCallMetaStore,
...
export function inviteDedupKey(callId: string, targetUserIds: string[]): string {
    if (!targetUserIds || targetUserIds.length === 0) return callId;
...
interface CrossNodeDepartedPayload {
    callId: string;
    departedClientId: string;
    callerId: string;
    lobbyName: string;
    sourceNodeId?: string;
    /** Document calls: the origin replica already told every participant
     *  (cluster-wide, through sendToClient), so receivers only prune their
     *  local bookkeeping and send nothing. */
    notified?: boolean;
...
function normalizeDocumentIds(raw: unknown, hostDocumentId: string): string[] {
    const out: string[] = [];
    if (hostDocumentId) out.push(hostDocumentId);
...
function normalizeTitles(raw: unknown): Record<string, string> | null {
 ...
 *  server-side. */
function publicMeta(meta: DocumentCallMeta): Omit<DocumentCallMeta, 'clients'> {
    const { clients: _clients, ...rest } = meta;
    return rest;
}

export class CallService {
    private static INVITE_TTL_MS = 60_000;
    private static INVITE_SWEEP_INTERVAL_MS = 15_000;
     ...
     *  conversation). */
    private static INVITE_TTL_SEC = 60;
    private static ACCEPTED_CALL_TTL_SEC = 4 * 60 * 60;
    ...
    private callEndedHook: NonNullable<CallConfig['onCallEnded']> | null;
    private callStartedHook: NonNullable<CallConfig['onCallStarted']> | null;
    private callMissedHook: NonNullable<CallConfig['onCallMissed']> | null;
    ...
    private inviteSweepTimer: ReturnType<typeof setInterval> | null = null;
    ...
    private activeInvitesByUserId = new Map<string /*userId*/, Set<string /*callId*/>>();
    private acceptedCallIds = new Set<string>();
    private recentInvites = new Map<string, number>();
    private crossNodePubSub: CallCrossNodePubSub | null;
    private crossNodeUnsubscribe: (() => void) | null = null;
    ...
    private stateStore: import('./CallStateStore').CallStateStore | null;
    /**
     ...
     */
    private sweeperIsLeader: CallSweeperIsLeader | null = null;
    ...
    private metaStore: DocumentCallMetaStore | null;
    private onOfflineInviteHook: CallServiceOptions['onOfflineInvite'] | null;
    private isClientAliveHook: CallServiceOptions['isClientAlive'] | null;
    /** Document calls — `<callId>|<userId>` → timer that turns a
     *  `reconnecting` participant into `left` when the grace runs out. */
    private docLeaveTimers = new Map<string, ReturnType<typeof setTimeout>>();
    /** Calls whose registration has reached the shared store at least once —
     *  after that, "not in the store" means gone. */
    private storeMirrored = new Set<string>();
    ...
    constructor(opts: CallServiceOptions) {
        ...
        this.inviteSweepTimer = setInterval(() => {
            ...
            if (this.sweeperIsLeader && !this.sweeperIsLeader()) {
                if (this.onSweepSkipped) {
                    try { this.onSweepSkipped('not_leader'); }
        ...
        if (typeof (this.inviteSweepTimer as any).unref === 'function') {
            (this.inviteSweepTimer as any).unref();
        }
        ...
        if (this.crossNodePubSub) {
            this.crossNodeUnsubscribe = this.crossNodePubSub.subscribe(
                CROSS_NODE_DEPARTED_TOPIC,
                (payload: string) => {
                    try {
                        const evt = JSON.parse(payload) as CrossNodeDepartedPayload;
     ...
     */
    private async handleCrossNodeDeparted(evt: CrossNodeDepartedPayload): Promise<void> {
        ...
        if (evt.callId) candidateCallIds.add(evt.callId);
        if (this.stateStore && typeof this.stateStore.getCallsForClient === 'function') {
            try {
                ...
                for (const id of cluster) candidateCallIds.add(id);
            } catch (e: any) {
                this.logger.warn(
                    `[CallService] cross-node getCallsForClient lookup failed for ${evt.departedClientId}: ${e?.message ?? e}`,
                );
            }
        ...
        if (localFromMap) {
            for (const id of localFromMap) candidateCallIds.add(id);
        }

        if (candidateCallIds.size === 0) return;
        ...
        this.logger.info(
            `[CallService] cross-node departure of ${evt.departedClientId} (origin hint=${evt.callId ?? '-'}) — covered ${candidateCallIds.size} call(s); notified ${totalNotified} local peer(s) total`,
        );
    }
     ...
     */
    private async notifyLocalPeersOfDeparture(
        callId: string,
        departedClientId: string,
        ...
        if (alreadyNotified) {
            ...
            if (callContinues) {
                if (local && departedClientId) local.participantClientIds.delete(departedClientId);
                const cs = departedClientId ? this.clientToCalls.get(departedClientId) : undefined;
                if (cs) {
                    cs.delete(callId);
                    if (cs.size === 0) this.clientToCalls.delete(departedClientId);
                }
            } else if (local) {
        ...
        if (this.stateStore) {
            try {
                ...
                this.logger.warn(
                    `[CallService] cross-node getCall lookup failed for ${callId}: ${e?.message ?? e}`,
                );
            }
        }
        ...
        if (participantClientIds.length === 0) return 0;
        ...
        for (const cid of participantClientIds) {
            ...
            if (isLive) {
                ...
                if (localState?.participantClientIds.has(cid)) localPeers.push(cid);
            }
        }
        ...
        for (const peerClientId of localPeers) {
            try {
                ...
                this.logger.warn(
                    `[CallService] cross-node notify failed for peer ${peerClientId} of call ${callId}: ${e?.message ?? e}`,
                );
            }
        }
        if (callContinues) {
    ...
    async dispose(): Promise<void> {
        ...
        if (this.crossNodeUnsubscribe) {
            try { this.crossNodeUnsubscribe(); } catch { /* ignore */ }
            this.crossNodeUnsubscribe = null;
        }
        if (this.inviteSweepTimer) {
            try { clearInterval(this.inviteSweepTimer); } catch { /* ignore */ }
        ...
        if (this.stateStore) {
            ...
            if (typeof this.stateStore.addClientToCall === 'function') {
                ...
                void this.stateStore.addClientToCall(clientId, callId, ttl)
                    .catch((e: any) => this.logger.warn(`[CallService] stateStore.addClientToCall failed for ${clientId}/${callId}: ${e?.message ?? e}`));
            }
        }
    }
     ...
     */
    private _emitCallStarted(summary: {
        callId: string;
        lobbyName: string;
        callerId: string;
        callerName?: string;
        ...
        if (!this.callStartedHook) return;
        try {
            void Promise.resolve(this.callStartedHook(summary)).catch((e: any) =>
                this.logger.warn(
                    `[CallService] onCallStarted failed for ${summary.callId}: ${e?.message ?? e}`,
                ),
            );
        } catch (e: any) {
            this.logger.warn(
                `[CallService] onCallStarted threw for ${summary.callId}: ${e?.message ?? e}`,
            );
        }
    }
    ...
    private forgetCall(callId: string, missedReason: 'cancelled' | 'declined' | 'no-answer' = 'cancelled'): void {
        ...
        if (this.metaStore) {
            void this.metaStore.delete(callId).catch(() => { /* best-effort */ });
        }
        for (const key of Array.from(this.docLeaveTimers.keys())) {
            if (key.startsWith(`${callId}|`)) this.clearDocLeaveTimerKey(key);
        }
        ...
        if (this.stateStore) {
            if (typeof this.stateStore.forgetLobbyCall === 'function' && state.lobbyName) {
                void this.stateStore.forgetLobbyCall(state.lobbyName, callId)
                    .catch(() => { /* index hygiene is best-effort */ });
            }
        ...
        if (this.stateStore) {
            void this.stateStore.forgetCall(callId)
                .catch((e: any) => this.logger.warn(`[CallService] stateStore.forgetCall failed for ${callId}: ${e?.message ?? e}`));
            if (typeof this.stateStore.removeClientFromCall === 'function') {
                for (const cid of departedParticipants) {
                    void this.stateStore.removeClientFromCall(cid, callId)
                        .catch((e: any) => this.logger.warn(`[CallService] stateStore.removeClientFromCall failed for ${cid}/${callId}: ${e?.message ?? e}`));
                }
            }
        }
    }
    ...
    private async clearInviteRegistryForCallStore(callId: string): Promise<void> {
        if (!this.stateStore || typeof this.stateStore.clearInviteForUser !== 'function') return;
        const state = this.activeCalls.get(callId);
        const targets = state?.originalTargetUserIds ?? state?.targetUserIds ?? [];
        for (const userId of targets) {
            try { await this.stateStore.clearInviteForUser(userId, callId); } catch { /* */ }
        }
    }
    ...
    private checkRecentInviteLocal(key: string): boolean {
        const now = Date.now();
        const lastSeen = this.recentInvites.get(key);
        if (lastSeen && (now - lastSeen) < INVITE_DEDUP_WINDOW_MS) return false;
        ...
        if (this.recentInvites.size > INVITE_DEDUP_MAX_ENTRIES) {
            ...
            for (const [k, ts] of this.recentInvites) {
                if (ts < cutoff) this.recentInvites.delete(k);
            }
        }
        return true;
    }

    async replayActiveInvitesForUser(clientId: string, userId: string): Promise<void> {
        ...
        if (callIds.length === 0) {
            ...
            if (local && local.size > 0) callIds = Array.from(local);
        }
        ...
        for (const callId of callIds) {
            ...
            let state: ActiveCallState | null = this.activeCalls.get(callId) ?? null;
            ...
            if (!state) {
                ...
                const localSet = this.activeInvitesByUserId.get(userId);
                if (localSet) localSet.delete(callId);
            ...
            const data: CallInvite = {
                ...
                replayed: true,
                originalTimestamp:
                    typeof state.invitedAt === 'number' ? new Date(state.invitedAt).toISOString() : undefined,
            };
            ...
            try {
                ...
                this.logger.warn(
                    `[CallService] replay invite send failed for clientId=${clientId} callId=${callId}: ${e?.message ?? e}`,
                );
            }
        }
        const remainLocal = this.activeInvitesByUserId.get(userId);
        if (remainLocal && remainLocal.size === 0) this.activeInvitesByUserId.delete(userId);
     ...
     */
    private async handleStatusQuery(clientId: string, payload: CallInvite): Promise<void> {
     ...
     */
    private async pushRosterToRinging(callId: string): Promise<void> {
        ...
        try {
            ...
            if (data.callId !== callId) return; // another call is the lobby's answer now
    ...
    private async buildActiveCallData(lobbyName: string): Promise<Record<string, unknown>> {
        const now = Date.now();
        ...
        const liveParticipants = async (ids: string[]): Promise<string[]> => {
            const out: string[] = [];
            for (const cid of ids) {
                ...
                if (local === null && this.isClientAliveHook) {
                    ...
                    catch { alive = true; /* unknown is not dead */ }
                    if (!alive) continue;
                }
                out.push(cid);
            }
        ...
        // 2026-09-25 (lane C, stale call id) — collect EVERY candidate for
        // this lobby, from this node's cache and the cluster lobby index, then
        // choose. Before, the first local match (Map order = the oldest) won
        // without checking the call still existed cluster-wide, so a call
        // whose state had been deleted kept answering `status` while a newer
        // live call on the same document was never offered.
        type Candidate = {
            id: string; callerId: string; callerName: string | null; startedAt: number | null;
            participantClientIds: string[]; targetUserIds: string[]; meta: DocumentCallMeta | null;
        };
        ...
        for (const [id, state] of this.activeCalls) {
            ...
            if (
                typeof state.inviteExpiresAt === 'number'
        ...
        if (this.stateStore && typeof this.stateStore.getCallIdsByLobby === 'function') {
            try {
                for (const id of await this.stateStore.getCallIdsByLobby(lobbyName)) candidateIds.add(id);
            } catch (e: any) {
        ...
        for (const id of candidateIds) {
            ...
            let viewKnown = false;
            if (this.stateStore) {
                try { view = await this.stateStore.getCall(id); viewKnown = true; }
            ...
            if (viewKnown && !view) {
                ...
                if (!young) {
                    ...
                    if (this.stateStore && typeof this.stateStore.forgetLobbyCall === 'function') {
                        void this.stateStore.forgetLobbyCall(lobbyName, id).catch(() => { /* best-effort */ });
                    }
                    continue;
                }
            }
            const ids = new Set<string>(local ? Array.from(local.participantClientIds) : []);
            ...
            if (alive.length === 0) {
                ...
                if (local) this.forgetCall(id);
                else if (view) await this.reapDeadStoredCall(id, lobbyName, view);
                continue;
            }
        ...
        const foundCallId: string | null = best?.id ?? null;
        ...
        const data: Record<string, unknown> = { lobbyName, active: !!foundCallId };
        if (foundCallId) {
            ...
            if (userIds.size === 0) {
                if (callerId) userIds.add(callerId);
                for (const uid of targetUserIds) if (uid) userIds.add(uid);
            }
    ...
    async handleCallEvent(
        ...
        let docMeta: DocumentCallMeta | null = null;
        let docRecipients: string[] | null = null;
        ...
        if (
            ...
            if (slug) {
                const state: ParticipantStateBroadcast = isParticipantStateBroadcast(payload) ? payload : {};
        ...
        if (action === 'invite' && callId) {
            ...
            if (this.stateStore && typeof this.stateStore.markRecentInvite === 'function') {
                try {
                    ...
                    this.logger.warn(
                        `[CallService] markRecentInvite failed for ${callId}: ${e?.message ?? e} — falling back to local dedup`,
                    );
            ...
            if (!isFirstInvite) {
                ...
                this.logger.info(
                    `[CallService] suppressing duplicate invite for callId=${callId} (wouldHaveBeenRecipients=${wouldHaveBeenRecipients})`,
                );
                return;
            }
        }
        ...
        if (action === 'invite' && this.canCallHook) {
            ...
            try {
                ...
                this.logger.debug(
                    `[CallService.canCall] check { caller: ${callerForPolicy}, targets: [${targetUserIds.join(', ')}], result: ${allowed} }`,
                );
                if (!allowed) {
                    this.logger.warn(
        ...
        let docInvite: { ringTargets: string[] } | null = null;
        if (
            action === 'invite' && this.metaStore && callId
        ...
        if (action === 'accepted' && callId && !resolvedLobbyName) {
            ...
            if (local?.lobbyName) {
                ...
                try {
                    const view = await this.stateStore.getCall(callId);
                    if (view?.lobbyName) resolvedLobbyName = view.lobbyName;
                } catch { /* */ }
            }
        }
        const shouldRegister = !!callId && (
            (action === 'invite' && !!lobbyName)
            || action === 'accepted'
        );
        if (shouldRegister) {
            ...
            if (this.stateStore) {
                ...
                if (!senderUserId && typeof this.messageRouter.getUserIdForClient === 'function') {
                    try {
                        senderUserId = (await Promise.resolve(
                ...
                if (senderUserId && typeof this.stateStore.registerUserCall === 'function') {
                    void this.stateStore.registerUserCall(senderUserId, callId!, CallService.ACCEPTED_CALL_TTL_SEC)
                        .catch((e: any) => this.logger.warn(`[CallService] registerUserCall failed for ${senderUserId}/${callId}: ${e?.message ?? e}`));
                }
                if (resolvedLobbyName && typeof this.stateStore.registerLobbyCall === 'function') {
                    void this.stateStore.registerLobbyCall(resolvedLobbyName, callId!, CallService.ACCEPTED_CALL_TTL_SEC)
                        .catch((e: any) => this.logger.warn(`[CallService] registerLobbyCall failed for ${resolvedLobbyName}/${callId}: ${e?.message ?? e}`));
                }
            }
            if (action === 'invite') {
                const state = this.activeCalls.get(callId);
                if (state) {
                    ...
                    if (typeof payload.callerName === 'string') {
                        state.originalCallerName = payload.callerName as string;
                    }
                ...
                if (this.stateStore && typeof this.stateStore.setInviteMetadata === 'function') {
                    ...
                    const meta: { invitedAt: number; callerName?: string } = {
                        invitedAt: state?.invitedAt ?? Date.now(),
                    };
                    if (callerNameRaw.length) meta.callerName = callerNameRaw;
                    void this.stateStore.setInviteMetadata(callId, meta)
                        .catch((e: any) => this.logger.warn(`[CallService] stateStore.setInviteMetadata failed for ${callId}: ${e?.message ?? e}`));
                }
                for (const targetUserId of (docInvite ? docInvite.ringTargets : targetUserIds)) {
                    ...
                    if (this.stateStore && typeof this.stateStore.registerInvite === 'function') {
                        ...
                        void this.stateStore.registerInvite(targetUserId, callId, expiresAtMs)
                            .catch((e: any) => this.logger.warn(`[CallService] stateStore.registerInvite failed for ${targetUserId}/${callId}: ${e?.message ?? e}`));
                    }
                }
                ...
                if (state && this.stateStore && typeof this.stateStore.setCall === 'function') {
                    const view: import('./CallStateStore').ActiveCallStateView = {
                        callerId: state.callerId,
                        lobbyName: state.lobbyName,
                        targetUserIds: state.originalTargetUserIds ?? state.targetUserIds,
                    ...
                    void this.stateStore.setCall(callId, view, CallService.INVITE_TTL_SEC)
                        .catch((e: any) => this.logger.warn(`[CallService] stateStore.setCall failed for ${callId}: ${e?.message ?? e}`));
                }
            }
            ...
            if (this.persistBindingHook) {
                void Promise.resolve(this.persistBindingHook({
                    callId: callId!, callerId, lobbyName: resolvedLobbyName, clientId, action: action as 'invite' | 'accepted',
                })).catch((e: any) => {
                    this.logger.warn(
                        `[CallService] persistCallBinding failed for ${action} callId=${callId}: ${e?.message ?? e}`,
                    );
                });
            }
        }
        ...
        if (action === 'accepted' && callId && wasFirstAccepted) {
            ...
            if (acceptedState) acceptedState.acceptedAt = acceptedAt;
            this._emitCallStarted({
                callId,
                ...
                participantClientIds: Array.from(
                    acceptedState?.everParticipated ?? acceptedState?.participantClientIds ?? [clientId],
                ),
            });
            ...
            if (this.stateStore && typeof this.stateStore.markAccepted === 'function') {
                void this.stateStore.markAccepted(callId, CallService.ACCEPTED_CALL_TTL_SEC)
                    .catch((e: any) => this.logger.warn(`[CallService] stateStore.markAccepted failed for ${callId}: ${e?.message ?? e}`));
            }
        }
        ...
        if (callId && (action === 'ended' || action === 'declined' || action === 'cancelled')) {
            ...
            if (state) {
                ...
                if (action === 'declined') state.declined = true;
                state.participantClientIds.delete(clientId);
                const cs = this.clientToCalls.get(clientId);
                if (cs) {
                    cs.delete(callId);
                    if (cs.size === 0) this.clientToCalls.delete(clientId);
                }
        ...
        if (fanoutTargets.length > 0) {
            ...
            if (action === 'accepted' && typeof this.messageRouter.getUserIdForClient === 'function') {
                ...
                if (accepterUserId && typeof this.messageRouter.getClientsByUserId === 'function') {
                    try {
                        const siblingMatches = await Promise.resolve(
                            this.messageRouter.getClientsByUserId([accepterUserId], clientId),
                        );
                        const siblingIds = Array.isArray(siblingMatches)
                            ? siblingMatches.map((m: { clientId: string }) => m.clientId)
                            : [];
     ...
     */
    async findClientsForUsers(userIds: string[], excludeClientId: string): Promise<string[]> {
        if (!this.messageRouter || typeof this.messageRouter.getClientsByUserId !== 'function') {
            return [];
        }
        const matches = await Promise.resolve(
            this.messageRouter.getClientsByUserId(userIds, excludeClientId),
        );
        ...
        for (const m of matches as Array<{ clientId: string }>) {
            if (isLive) {
        ...
        if (dropped.length > 0) {
            this.logger.warn(
                `[CallService] findClientsForUsers dropped ${dropped.length} non-OPEN local client(s): [${dropped.join(', ')}]`,
            );
        }
    ...
    async handleDisconnect(clientId: string): Promise<void> {
        ...
        if (this.roomMembershipMirrored.size > 0) {
            ...
            for (const key of this.roomMembershipMirrored) {
                if (key.endsWith(suffix)) this.roomMembershipMirrored.delete(key);
            }
        }
        ...
        if (localSet) for (const id of localSet) callIdSet.add(id);
        if (this.stateStore && typeof this.stateStore.getCallsForClient === 'function') {
            try {
                const cluster = await this.stateStore.getCallsForClient(clientId);
                for (const id of cluster) callIdSet.add(id);
            } catch (e: any) {
        ...
        for (const callId of callIdSet) {
            ...
            if (state) {
                ...
                this.logger.info(
                    `CallService: client ${clientId} dropped; sent ${(callContinues || graceDeferred) ? "'user-status: left'" : "synthetic 'ended'"} to ${state.participantClientIds.size - 1} local peer(s) of call ${callId} (remaining=${remainingAfterDeparture}${graceDeferred ? `, rejoin grace ${this.rejoinGraceMs}ms` : ''})`,
                );
            } else if (this.stateStore) {
                ...
                this.logger.info(
                    `CallService: client ${clientId} dropped; no local state for call ${callId} — cluster-only, publishing cross-node departure`,
                );
            }
            ...
            if (this.crossNodePubSub) {
                try {
                    const payload: CrossNodeDepartedPayload = {
                        callId,
     ...
     */
    async runInviteSweep(now: number = Date.now()): Promise<void> {
        if (this.sweepRunning) return;
        this.sweepRunning = true;
        try {
            let docCallIds = new Set<string>();
            if (this.metaStore) {
                try { docCallIds = new Set(await this.metaStore.listCallIds()); }
                catch (e: any) { this.logger.warn(`[CallService] meta listCallIds failed: ${e?.message ?? e}`); }
            }
            await this.sweepLegacyInvites(now, docCallIds);
            if (this.metaStore) {
                for (const callId of docCallIds) {
                    try { await this.sweepDocumentCall(callId, now); }
                    catch (e: any) { this.logger.warn(`[CallService] document-call sweep failed for ${callId}: ${e?.message ?? e}`); }
                }
            }
            const dedupCutoff = now - INVITE_DEDUP_WINDOW_MS;
            for (const [k, ts] of this.recentInvites) {
                if (ts < dedupCutoff) this.recentInvites.delete(k);
            }
        } finally {
    ...
    private async sweepLegacyInvites(now: number, docCallIds: Set<string>): Promise<void> {
        const expiredCallIds = new Set<string>();
        const expiredTargets: Array<{ userId: string; callId: string }> = [];
        for (const [userId, callIds] of this.activeInvitesByUserId) {
            for (const callId of callIds) {
                const state = this.activeCalls.get(callId);
                if (!state) { callIds.delete(callId); continue; }
                if (!(typeof state.inviteExpiresAt === 'number' && now > state.inviteExpiresAt)) continue;
        ...
        for (const callId of expiredCallIds) this.forgetCall(callId, 'no-answer');
        for (const [userId, callIds] of this.activeInvitesByUserId) {
            if (callIds.size === 0) this.activeInvitesByUserId.delete(userId);
        }
    }
    ...
    private async callHasAcceptedParticipant(callId: string, state?: ActiveCallState): Promise<boolean> {
        ...
        if (this.stateStore) {
            try {
                const view = await this.stateStore.getCall(callId);
                if (view && view.participantClientIds.length > 1) return true;
            } catch { /* local view stands */ }
        }
        return false;
    }
    ...
    private dropInviteForUser(userId: string, callId: string): void {
        const set = this.activeInvitesByUserId.get(userId);
        if (set) {
            set.delete(callId);
            if (set.size === 0) this.activeInvitesByUserId.delete(userId);
        }
        if (this.stateStore && typeof this.stateStore.clearInviteForUser === 'function') {
            void this.stateStore.clearInviteForUser(userId, callId).catch(() => { /* best-effort */ });
        }
    }

    /** Meta half of the sweep, for one document call. */
    private async sweepDocumentCall(callId: string, now: number): Promise<void> {
        ...
        for (const [userId, inv] of Object.entries(meta.invites)) {
            if (inv.state !== 'ringing' || now - inv.at <= CallService.INVITE_TTL_MS) continue;
            const missed: DocumentCallInvite = { ...inv, state: 'missed' };
        ...
        if (pruned === 'ended') return;
        if (changed || pruned === 'changed') {
            const fresh = await store.get(callId);
            if (fresh) await this.broadcastCallMeta(fresh);
        }
    }
     ...
     */
    private async pruneDeadClients(callId: string, meta: DocumentCallMeta): Promise<'none' | 'changed' | 'ended'> {
        if (!this.isClientAliveHook) return 'none';
        ...
        for (const cid of members) {
            ...
            if (!alive) dead.push(cid);
        }
        if (dead.length === 0) return 'none';
        for (const cid of dead) this.removeClientFromCallEverywhere(callId, cid);
        const survivors = members.filter((c) => !dead.includes(c));
        if (survivors.length === 0) {
            await this.endDocumentCall(callId, meta, 'participants-lost');
            return 'ended';
        }
        const survivorUsers = new Set(survivors.map((c) => meta.clients?.[c]).filter(Boolean) as string[]);
        let presentingCleared = false;
        for (const cid of dead) {
            const uid = meta.clients?.[cid];
            if (!uid || survivorUsers.has(uid)) continue;
    ...
    private async resolveActorUserId(clientId: string, payload: CallInvite, allowCallerId: boolean): Promise<string> {
        if (typeof this.messageRouter.getUserIdForClient === 'function') {
            try {
        ...
        if (allowCallerId && typeof payload.callerId === 'string') return payload.callerId;
        return '';
    }

    private async getDocumentMeta(callId: string | undefined): Promise<DocumentCallMeta | null> {
        if (!this.metaStore || !callId) return null;
        try { return await this.metaStore.get(callId); }
        catch (e: any) {
    ...
    private isDocParticipant(meta: DocumentCallMeta, userId: string, clientId: string): boolean {
        if (userId && meta.hostUserId === userId) return true;
        if (userId && meta.invites[userId]?.state === 'accepted') return true;
     ...
     *  node's cache. */
    private async docCallMemberClientIds(callId: string): Promise<string[]> {
        const out = new Set<string>();
        const local = this.activeCalls.get(callId);
        if (local) for (const cid of local.participantClientIds) out.add(cid);
        if (this.stateStore) {
            try {
                const view = await this.stateStore.getCall(callId);
                if (view) for (const cid of view.participantClientIds) out.add(cid);
            } catch { /* local view stands */ }
        }
        return Array.from(out);
    }

    private removeClientFromCallEverywhere(callId: string, clientId: string): void {
        const state = this.activeCalls.get(callId);
        if (state) state.participantClientIds.delete(clientId);
        const cs = this.clientToCalls.get(clientId);
        if (cs) {
        ...
        if (this.stateStore) {
            void this.stateStore.removeParticipant(callId, clientId).catch(() => { /* best-effort */ });
            if (typeof this.stateStore.removeClientFromCall === 'function') {
                void this.stateStore.removeClientFromCall(clientId, callId).catch(() => { /* best-effort */ });
            }
        }
    }

    private async sendToClients(clientIds: string[], envelope: unknown): Promise<void> {
        await Promise.allSettled(Array.from(new Set(clientIds)).map((cid) => {
            try { return Promise.resolve(this.messageRouter.sendToClient(cid, envelope)); }
            catch (err) { return Promise.reject(err); }
        }));
    }

    /** Every connection of the given users, cluster-wide. */
    private async sendToUsers(userIds: string[], envelope: unknown): Promise<void> {
        const ids = userIds.filter(Boolean);
        if (ids.length === 0 || typeof this.messageRouter.getClientsByUserId !== 'function') return;
        let matches: UserClientMatchLike[] = [];
        try {
    ...
    private userStatusEnvelope(callId: string, meta: DocumentCallMeta, userId: string, status: 'left' | 'reconnecting', reason: string, extra: Record<string, unknown> = {}): CallEvent {
        return {
            type: 'call',
            action: 'user-status',
            data: {
    ...
    private async broadcastCallMeta(meta: DocumentCallMeta, extraClientIds: string[] = []): Promise<void> {
        const members = await this.docCallMemberClientIds(meta.callId);
        const envelope: CallEvent = {
            type: 'call',
            action: 'call-meta',
            data: publicMeta(meta) as unknown as CallInvite,
            timestamp: new Date().toISOString(),
        };
     ...
     */
    private async handleDocumentInvite(
        clientId: string,
        callerUserId: string,
        payload: CallInvite,
        targetUserIds: string[],
    ): Promise<{ ringTargets: string[] }> {
        const store = this.metaStore!;
        ...
        if (!meta) {
            ...
            await store.set(meta);            await this.pruneLobbyIndex(documentId, callId);
        }
        if (callerUserId) await store.markClient(callId, clientId, callerUserId);

        const ring = payload.ring !== false;
        const ringTargets: string[] = [];
        for (const target of targetUserIds) {
            if (!target || target === callerUserId) continue;
            ...
            if (state === 'ringing') {
                ...
                const offline: DocumentCallOfflineInvite = {
                    callId,
                ...
                if (typeof payload.callerName === 'string') offline.callerName = payload.callerName;
                if (typeof payload.message === 'string' && payload.message) offline.message = payload.message.slice(0, 280);
                try { await Promise.resolve(this.onOfflineInviteHook(target, offline)); }
                catch (e: any) { this.logger.warn(`[CallService] onOfflineInvite failed for ${target}/${callId}: ${e?.message ?? e}`); }
            }
        }
        const fresh = await store.get(callId);
    ...
    private isUnmirroredYoung(callId: string, now: number): boolean {
        const local = this.activeCalls.get(callId);
        if (!local || this.storeMirrored.has(callId)) return false;
    ...
    private async pruneLobbyIndex(lobbyName: string, keepCallId: string): Promise<void> {
        const st = this.stateStore;
        if (!st || typeof st.getCallIdsByLobby !== 'function' || typeof st.forgetLobbyCall !== 'function') return;
        try {
            for (const id of await st.getCallIdsByLobby(lobbyName)) {
                if (id === keepCallId) continue;
                if (await st.getCall(id)) continue;
                if (this.isUnmirroredYoung(id, Date.now())) continue;
                await st.forgetLobbyCall(lobbyName, id);
                if (this.activeCalls.has(id)) this.forgetCall(id);
    ...
    private async handleDocumentCallAction(clientId: string, action: CallAction, payload: CallInvite): Promise<void> {
        if (!this.metaStore) {
        ...
        if (action === 'set-documents') {
            ...
            const titles = normalizeTitles(payload.documentTitles);
            const patch: { documentIds: string[]; documentTitles?: Record<string, string> } = { documentIds };
            if (titles || meta.documentTitles) {
                const merged = { ...(meta.documentTitles ?? {}), ...(titles ?? {}) };
                for (const k of Object.keys(merged)) if (!documentIds.includes(k)) delete merged[k];
     ...
     */
    private async handleDocumentCallVerb(
        clientId: string,
        action: CallAction,
        payload: CallInvite,
        meta: DocumentCallMeta,
    ): Promise<{ handled: true } | { handled: false; recipientsIfUntargeted: string[] }> {
        const callId = meta.callId;
        const store = this.metaStore!;
        ...
        if (action === 'cancelled') {
            ...
            for (const t of targets) {
                if (meta.invites[t] && meta.invites[t].state !== 'accepted') await store.markInvite(callId, t, null);
                this.dropInviteForUser(t, callId);
            }
            const envelope: CallEvent = { type: 'call', action: 'cancelled', data: payload, timestamp: new Date().toISOString() };
    ...
    private async leaveDocumentCall(callId: string, meta: DocumentCallMeta, clientId: string, userId: string, reason: string): Promise<void> {
    ...
    private async endDocumentCall(callId: string, meta: DocumentCallMeta, reason: string, endedBy?: string): Promise<void> {
        const members = await this.docCallMemberClientIds(callId);
        const envelope: CallEvent = {
            type: 'call',
            action: 'ended',
        ...
        const ringing = Object.entries(meta.invites).filter(([, i]) => i.state === 'ringing').map(([u]) => u);
        if (ringing.length) await this.sendToUsers(ringing, envelope);
        for (const key of Array.from(this.docLeaveTimers.keys())) {
            if (key.startsWith(`${callId}|`)) this.clearDocLeaveTimerKey(key);
        }
        ...
        if (!hadLocal) {
            if (this.stateStore) {
                void this.stateStore.forgetCall(callId).catch(() => { /* best-effort */ });
                if (typeof this.stateStore.forgetLobbyCall === 'function' && meta.documentId) {
                    void this.stateStore.forgetLobbyCall(meta.documentId, callId).catch(() => { /* best-effort */ });
                }
            }
            if (this.metaStore) await this.metaStore.delete(callId).catch(() => { /* best-effort */ });
        }
        if (this.crossNodePubSub) {
            try {
                const p: CrossNodeDepartedPayload = {
     ...
     */
    private async handleDocumentDisconnect(callId: string, meta: DocumentCallMeta, clientId: string): Promise<void> {
        ...
        if (!stillThere) this.scheduleDocLeave(callId, userId);
        void this.pushRosterToRinging(callId);
    }

    private scheduleDocLeave(callId: string, userId: string): void {
        ...
        const fire = async () => {
            ...
            if (!userId || members.some((c) => meta.clients?.[c] === userId)) return; // came back
        ...
        if (this.rejoinGraceMs <= 0) { void fire(); return; }
        const timer = setTimeout(() => { void fire(); }, this.rejoinGraceMs);
        if (typeof (timer as any).unref === 'function') (timer as any).unref();
        this.docLeaveTimers.set(key, timer);
    }

    private clearDocLeaveTimer(callId: string, userId: string): void {
        this.clearDocLeaveTimerKey(`${callId}|${userId}`);
    }

    private clearDocLeaveTimerKey(key: string): void {
    ...
    getStats(): { stateful: true; activeCalls: number; trackedClients: number } {
        return {
            stateful: true,
    ...
    getResumableStateStore(): import('./CallStateStore').CallStateStore {
        ...
        const self = this;
        return {
            async registerParticipant() { /* read-only adapter */ },
            async removeParticipant() { return null; },
            async forgetCall() { /* read-only adapter */ },
            ...
            async getCallIdsByClient(clientId: string) {
