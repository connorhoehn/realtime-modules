...
export class AwarenessLedger {
    ...
    private readonly _ids = new Map<string, Map<string, Map<number, number>>>();
    ...
    remember(clientId: string, channel: string, updateB64: string): void {
        ...
        for (let i = 0; i < len; i++) {
            let yid: number; let clock: number; let state: string;
            try {
            ...
            const byConn = this._ids.get(channel) ?? new Map<string, Map<number, number>>();
            this._ids.set(channel, byConn);
            const ids = byConn.get(clientId) ?? new Map<number, number>();
            byConn.set(clientId, ids);
            // A null state is the client's own departure for that id: nothing
            // left to say on its behalf.
            if (state === 'null') ids.delete(yid);
            else ids.set(yid, clock);
        }
    }
    ...
    departure(clientId: string, channel: string): string | null {
        ...
        if (byConn && byConn.size === 0) this._ids.delete(channel);
    ...
    departures(clientId: string): Array<{ channel: string; update: string }> {
        const out: Array<{ channel: string; update: string }> = [];
        for (const channel of [...this._ids.keys()]) {
            const update = this.departure(clientId, channel);
            if (update) out.push({ channel, update });
        }
        return out;
    }
}
