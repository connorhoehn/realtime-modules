...
import { UpdateCoalescer } from 'distributed-core';
import { AWARENESS_BATCH_WINDOW_MS } from './config';
...
class AwarenessCoalescer {
    ...
    private logger: any;
    private _batches: Map<string, Map<string, string>>;
    private _coalescer: any;
    ...
    constructor(messageRouter: MessageRouterContract, logger: any) {
        ...
        this._coalescer = new UpdateCoalescer({
            windowMs: AWARENESS_BATCH_WINDOW_MS,
            onFlush: (channel: string) => this._broadcast(channel),
        });
    }
     ...
     */
    private async _broadcast(channel: string): Promise<void> {
        ...
        try {
            const merged: Array<{ clientId: string; update: string }> = [];
