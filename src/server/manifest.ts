...
export const crdtManifest: FeatureManifest = {
    ...
    envVars: {
        DYNAMODB_CRDT_TABLE: {
            ...
            description:
                'DDB table for CRDT snapshots (informational — the SnapshotStore adapter owns the real table name).',
        },
        DYNAMODB_DOCUMENTS_TABLE: {
            required: false,
            default: 'crdt-documents',
            description:
                'DDB table for document metadata (informational — the MetadataStore adapter owns the real table name).',
        },
        DDB_TABLE_PREFIX: {
            required: false,
            description:
                'Optional prefix applied to DDB table names so multiple consumers can share a DDB-local instance without collision.',
        },
        SNAPSHOT_DEBOUNCE_MS: {
            required: false,
            default: '5000',
            description:
                'Debounce window before writing a snapshot after the last CRDT update.',
        },
        SNAPSHOT_INTERVAL_MS: {
            required: false,
            default: '300000',
            description:
                'Interval for periodic snapshot sweeps across all active channels.',
        },
        IDLE_EVICTION_MS: {
            required: false,
            default: '600000',
            description:
                'Grace period before evicting an idle (0-subscriber) Y.Doc from memory.',
        },
        EVENT_BUS_NAME: {
            required: false,
            default: 'social-events',
            description:
                'EventBridge bus name (informational — only used by an EventBridge-backed SnapshotStore adapter, if any).',
        },
    },
