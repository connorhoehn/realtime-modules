...
import {
    ...
    type RoomErrorFrame,
    type RoomLogger,
    type RoomMemberLocal,
    type RoomMemberRecord,
    type RoomMessageRouter,
    type RoomOccupancyDelta,
    type RoomServerEvent,
    type RoomServiceOptions,
    type RoomMetricsHooks,
} from './types';
import type { RoomStateStore, RoomMemberSnapshot } from './RoomStateStore';
 ...
 */
function deriveOrgFromSlug(slug: string): string {
    if (!slug) return 'default-org';
...
export class RoomService {
    ...
    logger: RoomLogger;
    private stateStore: RoomStateStore | null;
    private crossNodePubSub: RoomCrossNodePubSub | null;
    private crossNodeUnsubscribe: (() => void) | null = null;
    private nodeId: string | null;
    private readonly metricsHooks: RoomMetricsHooks;

    private occupancyDebounceMs: number;
    private maxMembersPerDelta: number;
    private occupancyCacheTtlMs: number;
    ...
    private activeRooms = new Map<string, Map<string, RoomMemberLocal>>();
    /** Reverse index: which rooms is each LOCAL client a member of. */
    private clientToRooms = new Map<string, Set<string>>();
    ...
    private occupancyCache = new Map<string, { at: number; count: number; members: RoomMemberSnapshot[] }>();
    ...
    private clientToSubscriptions = new Map<string, Set<string>>();
    /** rooms:index subscriber set. */
    private indexSubscribers = new Set<string>();
    ...
    constructor(opts: RoomServiceOptions) {
        ...
        const config: RoomConfig = opts.config ?? {};
        ...
        if (this.crossNodePubSub) {
            this.crossNodeUnsubscribe = this.crossNodePubSub.subscribe(
                ...
                (payload: string) => {
                    try {
                        const evt = JSON.parse(payload) as CrossNodeRoomEvent;
    ...
    async dispose(): Promise<void> {
        if (this.crossNodeUnsubscribe) {
            try { this.crossNodeUnsubscribe(); } catch { /* */ }
            this.crossNodeUnsubscribe = null;
        }
        if (this.debounceTimer) {
            try { clearTimeout(this.debounceTimer); } catch { /* */ }
    ...
    async handleAction(clientId: string, action: string, data: RoomActionPayload | null | undefined): Promise<void> {
        try {
            if (!ALLOWED_ROOM_ACTIONS.has(action as RoomAction)) {
                this.sendError(clientId, `Unknown room action: ${action}`);
                return;
            }
            const typedAction = action as RoomAction;
            const payload: RoomActionPayload = data ?? {};
            switch (typedAction) {
                ...
                case 'join': {
                    ...
                    const clientData = this.messageRouter.getClientData?.(clientId) as
                ...
                case 'leave': {
                    ...
                    const clientData2 = this.messageRouter.getClientData?.(clientId) as
    ...
    private async handleSubscribeIndex(clientId: string): Promise<void> {
    ...
    private handleUnsubscribeIndex(clientId: string): void {
        this.indexSubscribers.delete(clientId);
    }

    private handleSubscribe(clientId: string, slug: string): void {
    ...
    private handleUnsubscribe(clientId: string, slug: string): void {
        const subs = this.roomSubscribers.get(slug);
        if (subs) {
            subs.delete(clientId);
            if (subs.size === 0) this.roomSubscribers.delete(slug);
        }
        const mine = this.clientToSubscriptions.get(clientId);
        if (mine) {
            mine.delete(slug);
            if (mine.size === 0) this.clientToSubscriptions.delete(clientId);
        }
    }
    ...
    async handleMemberJoined(
        ...
        if (!already) {
            try { this.metricsHooks.recordMemberJoined?.(deriveOrgFromSlug(slug)); } catch { /* */ }
            const count = room.size;
            this.logger.info(
                '[room] member joined',
                { slug, userId, clientId, participantId, roomCount: count },
            );
        }
    ...
    async handleMemberLeft(
        ...
        userId: string,
        clientId: string,
        reason: 'explicit' | 'disconnect' = 'explicit',
    ): Promise<void> {
        if (!slug || !clientId) return;
        ...
        if (room) {
            ...
            if (room.size === 0) this.activeRooms.delete(slug);
        }
        const mine = this.clientToRooms.get(clientId);
        if (mine) {
            mine.delete(slug);
            if (mine.size === 0) this.clientToRooms.delete(clientId);
        }
        ...
        if (removed) {
            ...
            try { this.metricsHooks.recordMemberLeft?.(deriveOrgFromSlug(slug), reason); } catch { /* */ }
            this.logger.info(
                '[room] member left',
                { slug, userId, reason, remainingCount },
            );
            const envelope: RoomServerEvent = {
                type: 'room',
                action: 'member-left',
    ...
    async handleDisconnect(clientId: string): Promise<void> {
        ...
        if (subs) {
            for (const slug of subs) {
                ...
                if (set) {
                    set.delete(clientId);
                    if (set.size === 0) this.roomSubscribers.delete(slug);
                }
            }
    ...
    private async fanOutToRoomSubscribers(
        slug: string,
        envelope: RoomServerEvent,
        excludeClientId: string | null,
    ): Promise<void> {
        const subs = this.roomSubscribers.get(slug);
    ...
    private schedulePendingDelta(slug: string): void {
    ...
    private async flushPendingDeltas(): Promise<void> {
        ...
        try { this.metricsHooks.setActiveCount?.(activeCount); } catch { /* */ }
        ...
        if (this.indexSubscribers.size === 0) {
            ...
            this.logger.debug(
                '[room] occupancy delta flushed',
                { deltaCount: this.pendingDeltas.size, subscriberCount: 0 },
            );
            this.pendingDeltas.clear();
            return;
        }
        ...
        this.pendingDeltas.clear();
        const deltas: RoomOccupancyDelta[] = [];
        ...
        this.logger.debug(
            '[room] occupancy delta flushed',
            { deltaCount: deltas.length, subscriberCount: this.indexSubscribers.size },
        );
    }

    private async computeOccupancyDelta(slug: string): Promise<RoomOccupancyDelta> {
        ...
        const trimmed = snapshots.slice(0, this.maxMembersPerDelta).map((m): RoomMemberRecord => ({
            userId: m.userId,
     ...
     */
    async getRoomOccupancyCount(slug: string): Promise<number> {
        if (!slug) return 0;
        const snapshots = await this.getRoomMembers(slug);
     ...
     * do today).
     */
    async getRoomMembers(slug: string): Promise<RoomMemberSnapshot[]> {
        if (!slug) return [];
        const now = Date.now();
        ...
        let members: RoomMemberSnapshot[] | null = null;
        if (this.stateStore) {
            try {
        ...
        if (members === null) {
            ...
            if (!room) {
                ...
                members = Array.from(room.values()).map((m): RoomMemberSnapshot => ({
                    clientId: m.clientId,
                    userId: m.userId,
                    displayName: m.displayName,
                    participantId: m.participantId,
    ...
    private async collectFullSnapshot(): Promise<RoomOccupancyDelta[]> {
        if (this.stateStore) {
            try {
                const rows = await this.stateStore.listAllRoomCounts();
                return rows.map((r) => ({
        ...
        const out: RoomOccupancyDelta[] = [];
        for (const [slug, room] of this.activeRooms) {
            if (room.size === 0) continue;
            const members = Array.from(room.values()).slice(0, this.maxMembersPerDelta).map((m): RoomMemberRecord => ({
                userId: m.userId,
                displayName: m.displayName,
                participantId: m.participantId,
            }));
    ...
    sendError(clientId: string, message: string): void {
        ...
        const frame: RoomErrorFrame = {
            type: 'error',
            service: 'room',
            message,
            timestamp: new Date().toISOString(),
        };
        try { this.messageRouter.sendToClient(clientId, frame); } catch { /* */ }
    }

    getStats(): { stateful: true; activeRooms: number; trackedClients: number; indexSubscribers: number; roomSubscribers: number } {
        let roomSubs = 0;
        for (const set of this.roomSubscribers.values()) roomSubs += set.size;
        return {
            stateful: true,
