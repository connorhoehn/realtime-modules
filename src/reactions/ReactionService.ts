...
export class ReactionService {
    ...
    async handleSendReaction(
        ...
        if (this.store && this._isTargeted(reaction)) {
            if (!reaction.userId) {
                this.sendError(
                    clientId,
                    'A reaction on a message needs an identified sender; this connection has none',
                );
                return;
            }
            try {
    ...
    async handleRemoveReaction(
        clientId: string,
        { channel, emoji, targetId }: { channel: string; emoji: string; targetId?: unknown },
    ): Promise<void> {
        if (!channel || !emoji) {
            this.sendError(clientId, 'Channel and emoji are required');
            return;
        ...
        if (history) {
            const kept = history.filter(
                (r) => !(r.targetId === targetId && r.emoji === emoji && r.userId === identity.userId),
            );
    ...
    _toStored(reaction: Reaction): StoredReaction {
        const stored: StoredReaction = {
            ...
            targetId: reaction.targetId as string,
            emoji: reaction.emoji,
            userId: reaction.userId as string,
            timestamp: reaction.timestamp,
        };
        if (reaction.displayName !== undefined) stored.displayName = reaction.displayName;
        return stored;
    }
    ...
    _fromStored(stored: StoredReaction): Reaction {
        ...
        if (stored.displayName !== undefined) reaction.displayName = stored.displayName;
        return reaction;
    }

    async _replayStoredReactions(clientId: string, channel: string): Promise<void> {
