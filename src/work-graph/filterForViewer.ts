...
const capabilities: ReadonlySet<CollaborationCapability> = new Set([
  'message',
  'view-document',
  'edit-document',
  'join-conversation',
...
function allowedDecision(
  decisions: ReadonlyMap<OpaqueWorkId, DisclosureDecision>,
  id: OpaqueWorkId,
...
function safeCapabilities(decision: DisclosureDecision): CollaborationCapability[] {
...
function viewerEdge(edge: InternalWorkEdge, decision: DisclosureDecision, nodes: ReadonlyMap<OpaqueWorkId, ViewerWorkNode>): ViewerWorkEdge | undefined {
  ...
  const disclosure = decision.maximumDisclosure;
  if (disclosure === 'existence') return undefined;
  if (disclosure !== 'summary' && disclosure !== 'details') return undefined;
 ...
 */
export function filterForViewer(state: WorkProjectionState, policy: WorkGraphPolicyInput): FilteredWorkGraph {
  ...
  if (
    state.organizationId !== policy.scope.organizationId
  ...
  const visibleEdges: ViewerWorkEdge[] = [];
  for (const edge of Object.values(state.edges)) {
    if (edge.deletedAt || edge.organizationId !== state.organizationId || edge.actorId !== state.actorId) continue;
    ...
    if (projected) visibleEdges.push(projected);
  }
