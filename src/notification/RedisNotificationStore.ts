...
export class RedisNotificationStore {
    ...
    private readonly listKeyPrefix: string;
    private readonly readKeyPrefix: string;
    ...
    constructor(opts: RedisNotificationStoreOpts) {
        ...
        this.maxPerUser = opts.maxPerUser ?? NOTIFICATION_MAX_PER_USER;
        this.ttlSec = opts.ttlSec ?? NOTIFICATION_TTL_SEC;
     ...
     */
    async append(userId: string, record: NotificationRecord): Promise<void> {
    ...
    private async readState(userId: string): Promise<Record<string, boolean>> {
        if (!this.redis) return {};
        try {
            const hash = await this.redis.hGetAll(this.readKey(userId));
            const map: Record<string, boolean> = {};
            for (const id of Object.keys(hash ?? {})) map[id] = true;
            return map;
        } catch (err: any) {
 ...
 */
function toScore(timestamp: string | undefined): number {
