...
// ---------------------------------------------------------------------------
// Outbound frame shapes — exactly the three the hook listens for
// (useNotifications.ts:11-21). `service: 'notification'` is included so the
// client's onMessage dispatcher can attribute the frame even though the hook
// keys on `type` alone.
// ---------------------------------------------------------------------------
...
export interface NotificationBulkUpdateFrame {
    ...
    service: 'notification';
    payload: { notifications: NotificationRecord[] };
}
...
// ---------------------------------------------------------------------------
// Collaborator contracts (structural — keeps the service decoupled from the
// concrete MessageRouter / redis client / logger implementations and makes
// unit-testing with fakes trivial).
// ---------------------------------------------------------------------------
...
export interface NotificationMessageRouter {
    ...
    ): Promise<Array<{ clientId: string; userId: string }>> | Array<{ clientId: string; userId: string }>;
