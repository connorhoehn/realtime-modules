...
export class NotificationService {
    ...
    async notifyUser(
    ...
    ): Promise<{ record: NotificationRecord; delivered: number }> {
        ...
        const frame: NotificationNewFrame = {
    ...
    private async handleMarkRead(userId: string, data: any): Promise<void> {
        ...
        const frame: NotificationReadFrame = {
    ...
    private async handleGetHistory(clientId: string, userId: string): Promise<void> {
        const notifications = await this.store.list(userId);
        const frame: NotificationBulkUpdateFrame = {
     ...
     */
    async replayUnreadForUser(clientId: string, userId: string): Promise<void> {
     ...
     */
    private async deliverToUser(userId: string, frame: unknown): Promise<number> {
        let matches: Array<{ clientId: string; userId: string }> = [];
        try {
            const res = await Promise.resolve(
                this.messageRouter.getClientsByUserId([userId], ''),
            );
        ...
        const isLive =
            typeof this.messageRouter.isClientLive === 'function'
     ...
     * notifications read.
     */
    private resolveUserId(clientId: string, data: any): string | null {
        const router = this.messageRouter as unknown as {
