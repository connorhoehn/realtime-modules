...
interface RunState<F> {
  /** The next number to release; null until the run's first frame. */
  next: number | null;
  held: Map<number, { frame: F; at: number }>;
  timer: unknown;
  /** Highest number released by a timeout (a later arrival below it is late). */
  timedOutBelow: number;
  timedOutAt: number;
}
...
function sequenceOf(frame: SequencedFrame): { runId: string; runSeq: number; pipelineId?: string } | null {
  const p = frame.payload as { runId?: unknown; runSeq?: unknown; pipelineId?: unknown } | null | undefined;
  if (!p || typeof p !== 'object') return null;
  if (typeof p.runId !== 'string' || !p.runId) return null;
  if (typeof p.runSeq !== 'number' || !Number.isInteger(p.runSeq) || p.runSeq < 1) return null;
...
export function synthesizeRunStarted<F extends SequencedFrame>(first: F): F {
  const p = (first.payload ?? {}) as Record<string, unknown>;
  const payload: Record<string, unknown> = { runId: p.runId, runSeq: 1, synthesized: true };
  for (const k of ['pipelineId', 'runOwnerId', 'triggeredBy', 'pipelineVersion']) if (p[k] !== undefined) payload[k] = p[k];
  ...
  return { ...first, eventType: 'pipeline.run.started', payload, emittedAt: first.emittedAt } as F;
}

export class RunEventSequencer<F extends SequencedFrame> {
  private readonly runs = new Map<string, RunState<F>>();
  private readonly opts: RunEventSequencerOptions<F>;
  ...
  gapWaitMs(): number {
    if (typeof this.opts.gapMs === 'number') return this.opts.gapMs;
    const min = this.opts.minGapMs ?? 20;
    const max = this.opts.maxGapMs ?? 150;
    if (this.samples.length < MIN_SAMPLES) return max;
  ...
  push(frame: F): void {
    ...
    if (!seq) { this.opts.deliver(frame); return; }
    const run = this.runFor(seq.runId);
    const at = this.now();
    if (run.next === null && seq.runSeq === 1) run.next = 1;
    if (run.next !== null && seq.runSeq < run.next) {
      this.late += 1;
      // It missed a timeout: let the adaptive wait learn how late it was.
      if (seq.runSeq <= run.timedOutBelow) this.sample(this.gapWaitMs() + (at - run.timedOutAt));
    ...
    if (!run.held.has(seq.runSeq)) { run.held.set(seq.runSeq, { frame, at }); this.reordered += 1; }
    if (run.timer == null) {
  ...
  flushAll(): void {
    for (const run of this.runs.values()) this.release(run, false);
  }

  private runFor(runId: string): RunState<F> {
    let run = this.runs.get(runId);
    if (run) return run;
    run = { next: null, held: new Map(), timer: null, timedOutBelow: 0, timedOutAt: 0 };
    this.runs.set(runId, run);
    while (this.runs.size > (this.opts.maxRuns ?? 500)) {
      const [oldestKey, oldest] = this.runs.entries().next().value as [string, RunState<F>];
  ...
  private drain(run: RunState<F>, at: number): void {
    let oldestHold: number | null = null;
    ...
    if (oldestHold !== null) this.sample(at - oldestHold);
    if (run.held.size === 0 && run.timer != null) { this.clearTimer(run.timer); run.timer = null; }
  }

  private timeout(runId: string, run: RunState<F>): void {
    if (run.held.size === 0) return;
    const numbers = [...run.held.keys()].sort((a, b) => a - b);
    const from = run.next ?? 1;
    const missing: number[] = [];
    for (let n = from; n < numbers[numbers.length - 1]!; n += 1) if (!run.held.has(n)) missing.push(n);
  ...
  private release(run: RunState<F>, timedOut: boolean): boolean {
    if (run.timer != null) { this.clearTimer(run.timer); run.timer = null; }
    if (run.held.size === 0) return false;
    const numbers = [...run.held.keys()].sort((a, b) => a - b);
    ...
    if (run.next === null && this.opts.synthesizeStart !== false) {
      ...
      if (start) { this.opts.deliver(start); this.synthesized += 1; synthesizedStart = true; }
    }
    if (timedOut) this.gapsTimedOut += numbers.length;
