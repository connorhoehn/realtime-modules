...
export function useLVSSubscriber(
  ...
  const lastFailureWasTransientRef = useRef(true);
  ...
  // state change.
  const phaseRef = useRef<LVSSubscriberPhase>('idle');
  ...
  const ensureSlowPoll = useCallback(() => {
    if (slowPollTimerRef.current != null) return;
    slowPollTimerRef.current = setInterval(() => {
      if (stoppedRef.current) return;
      const p = phaseRef.current;
      const parked =
        p === 'waiting-for-producer' ||
        (p === 'failed' && lastFailureWasTransientRef.current);
      if (!parked) return;
      console.info(
        `[lvs-subscriber] slow-poll tick phase=${p} → start()`,
      );
  ...
  const scheduleReconnect = useCallback(() => {
    ...
    const delayMs =
      NETWORK_RECONNECT_DELAYS_MS[
        Math.min(attempt, NETWORK_RECONNECT_DELAYS_MS.length - 1)
      ];
    reconnectAttemptRef.current = attempt + 1;
    log(
      `subscriber reconnect in ${delayMs / 1000}s (attempt ${attempt + 1}/${MAX_NETWORK_RECONNECT_ATTEMPTS})`,
      'warn',
    );
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (status === 409) {
        ...
        // budget consumed.
        console.info(
          `[lvs-subscriber] WHEP 409 → waiting-for-producer (no ladder)`,
        );
        lastFailureWasTransientRef.current = true;
      ...
      if (status === 404 || status === 415) {
        ...
        console.info(
          `[lvs-subscriber] WHEP ${status} → terminal failed (client/server bug, no retry)`,
        );
      ...
      if (status === 503) {
        ...
        // budget consumed.
        const retryAfter = e instanceof LVSApiError ? e.retryAfterSec : null;
        const delaySec = retryAfter != null
          ? Math.max(1, Math.min(retryAfter, 300))
          : 5;
        console.info(
          `[lvs-subscriber] WHEP 503 → Retry-After ${delaySec}s (single retry, no ladder)`,
        );
        lastFailureWasTransientRef.current = true;
        setPhase('reconnecting');
        if (reconnectTimerRef.current != null) {
      ...
      console.info(
        `[lvs-subscriber] WHEP ${status ?? 'network'} → network ladder ` +
    ...
    const debouncedRewhep = () => {
      ...
      debounceTimer = setTimeout(() => {
        ...
        if (parked) {
          ...
          // start()).
          console.info(
            `[lvs-subscriber] producer.added while phase=${p} → start() directly`,
          );
          reconnectAttemptRef.current = 0;
          stoppedRef.current = false;
          void startRef.current().catch(() => { /* ignore */ });
          return;
        }
      ...
      ws.addEventListener('message', (ev) => {
        try {
          ...
          if (!parked) {
            ...
            const haveKind = Array.from(tracksRef.current.values()).some(
              (t) => t.kind === msg.kind,
            );
            if (haveKind) return;
          }
