...
export const AUDIO_VIDEO_SETTINGS_KEY = 'call-device-preferences';
...
export function readAudioVideoSettings(storage: StorageLike | null, key = AUDIO_VIDEO_SETTINGS_KEY): AudioVideoSettings {
  if (!storage) return { ...DEFAULT_AUDIO_VIDEO_SETTINGS };
  try {
    const raw = storage.getItem(key);
    const v = raw ? JSON.parse(raw) : null;
    if (!v || typeof v !== 'object') return { ...DEFAULT_AUDIO_VIDEO_SETTINGS };
    const out: AudioVideoSettings = { ...DEFAULT_AUDIO_VIDEO_SETTINGS };
    for (const k of ['microphoneId', 'speakerId', 'cameraId'] as const) {
      if (typeof v[k] === 'string' && v[k]) out[k] = v[k];
    }
    for (const k of ['noiseSuppression', 'echoCancellation', 'autoGainControl', 'mirrorPreview', 'rememberDevices'] as const) {
      if (typeof v[k] === 'boolean') out[k] = v[k];
    }
    if (typeof v.volume === 'number' && v.volume >= 0 && v.volume <= 1) out.volume = v.volume;
    if (['auto', '720p', '480p', '360p'].includes(v.quality)) out.quality = v.quality;
    if (v.frameRate === 'auto' || v.frameRate === 30 || v.frameRate === 15) out.frameRate = v.frameRate;
    return out;
  } catch {
    return { ...DEFAULT_AUDIO_VIDEO_SETTINGS };
  }
}

const QUALITY: Record<AudioVideoSettings['quality'], { width: number; height: number }> = {
...
export function audioVideoConstraints(s: AudioVideoSettings, video: boolean): MediaStreamConstraints {
  ...
  const v: MediaTrackConstraints = s.quality === 'auto'
    ? { width: { ideal: q.width }, height: { ideal: q.height } }
    : { width: { ideal: q.width, max: q.width }, height: { ideal: q.height, max: q.height } };
  if (s.frameRate !== 'auto') v.frameRate = { ideal: s.frameRate, max: s.frameRate };
  if (s.cameraId) v.deviceId = { ideal: s.cameraId };
...
async function playThroughSink(el: HTMLAudioElement, sinkId: string | undefined, volume: number): Promise<void> {
  ...
  if (sinkId && typeof withSink.setSinkId === 'function') {
    try { await withSink.setSinkId(sinkId); } catch { /* default output */ }
  }
  await el.play();
}

export function useAudioVideoSettings(opts: UseAudioVideoSettingsOptions = {}): UseAudioVideoSettingsResult {
  const storageKey = opts.storageKey ?? AUDIO_VIDEO_SETTINGS_KEY;
  const storage = opts.storage === undefined ? defaultStorage() : opts.storage;
  const md = opts.mediaDevices ?? (typeof navigator !== 'undefined' ? navigator.mediaDevices : undefined);

  const [settings, setSettings] = useState<AudioVideoSettings>(() => readAudioVideoSettings(storage, storageKey));
  const settingsRef = useRef(settings);
  settingsRef.current = settings;

  const update = useCallback((patch: Partial<AudioVideoSettings>) => {
    setSettings((prev) => {
      const next = { ...prev, ...patch };
      if (storage) {
        try {
          if (next.rememberDevices) storage.setItem(storageKey, JSON.stringify(next));
          // Unchecking "Use these devices next time" forgets what was stored.
          else if (prev.rememberDevices) storage.removeItem(storageKey);
  ...
  const constraints = useCallback((video: boolean) => audioVideoConstraints(settingsRef.current, video), []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPreviewError(name === 'NotAllowedError' ? 'denied' : name === 'NotFoundError' ? 'unavailable' : (err as Error)?.message || 'error');
        setPreviewStream(null);
      }
    })();
    return () => {
  ...
  useEffect(() => {
    ...
    if (!Ctor || !track) { setMicLevel(0); return; }
  ...
  // ---- test mic / test sound -----------------------------------------
  const [testMicState, setTestMicState] = useState<'idle' | 'recording' | 'playing'>('idle');
  const testMic = useCallback(async () => {
    if (!md || typeof MediaRecorder === 'undefined' || testMicState !== 'idle') return;
    ...
    try {
      ...
      const ended = new Promise<void>((resolve) => { el.onended = () => resolve(); el.onerror = () => resolve(); });
  ...
  const testSound = useCallback(async () => {
    ...
    try {
      ...
      if (dest && typeof Audio !== 'undefined') {
        ...
        gain.connect(dest);
        const el = new Audio();
        (el as HTMLAudioElement & { srcObject: MediaStream | null }).srcObject = dest.stream;
