 ...
 */
export function waitForIceGather(pc: RTCPeerConnection, timeoutMs = 3000): Promise<RTCIceGatheringState> {
 ...
 */
export function classifyNetQ(lossPct: number, hasRecentPackets: boolean): NetQuality {
  if (!hasRecentPackets) return 'dead';
  if (lossPct < 2) return 'good';
  if (lossPct < 5) return 'medium';
  return 'poor';
}
