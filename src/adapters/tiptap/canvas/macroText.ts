 ...
 */
export function macroTextFromData(name: string, data: JsonObject): string {
 ...
 */
export function macroDataFromText(name: string, text: string): JsonObject | null {
  const body = text === '' || text.endsWith('\n') ? text : `${text}\n`;
  const fence = fenceFor(body);
  try {
    const blocks = parseBlocks(`${fence}macro:${name}\n${body}${fence}\n`);
    const first = blocks[0];
    if (!first || first.type !== 'macro') return null;
    return first.data;
  } catch {
    return null;
  }
 ...
 */
export function minimalEdit(before: string, after: string): TextRangeEdit | null {
