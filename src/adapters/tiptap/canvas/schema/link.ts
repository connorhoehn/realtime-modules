 ...
 */
export const CANVAS_STARTER_KIT_OPTIONS = { link: false as const };
...
// Declared under `canvasLink` rather than `link`. `@tiptap/extension-link` already
// augments `Commands['link']` and reaches this program through StarterKit's types,
// so reusing that key is a duplicate-member compile error. Renaming the COMMANDS
// instead would make every toolbar call site differ from the rest of the Tiptap
// ecosystem for no gain; the namespace key is invisible to callers.
declare module '@tiptap/core' {
  // eslint-disable-next-line @typescript-eslint/consistent-type-definitions
  interface Commands<ReturnType> {
    canvasLink: {
      /** Link the current selection. Refuses an href the scheme filter rejects. */
      setLink: (attributes: { href: string; title?: string | null }) => ReturnType;
 ...
 */
export const DANGEROUS_SCHEMES: readonly string[] = ['javascript', 'data', 'vbscript'];
...
export function sanitizeHref(raw: unknown): string | null {
  ...
  if (scheme !== null && DANGEROUS_SCHEMES.includes(scheme)) return null;
  return href;
}
 ...
 */
export function linkTagAttributes(attrs: Record<string, unknown>): Record<string, string> {
 ...
 */
export function linkHrefFromPastedText(text: string): string | null {
...
export const CanvasLink = Mark.create<CanvasLinkOptions>({
  name: 'link',
  ...
  renderHTML({ HTMLAttributes, mark }) {
    return [
      'a',
      mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, linkTagAttributes(mark.attrs)),
      0,
    ];
  },
  ...
  addProseMirrorPlugins() {
    ...
    return [
      new Plugin({
        key: new PluginKey('canvasLinkPasteOverSelection'),
        props: {
